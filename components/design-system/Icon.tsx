...
const PATHS: Record<string, string | { d: string; fill?: string; stroke?: string }> = {
  ...
  minus: 'M5 12h14',
  star: { d: 'M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z', fill: 'currentColor', stroke: 'none' },
...
export function Icon({ name, size = 18, strokeWidth = 1.75, className, ...props }: IconProps) {
  ...
  const d = isFilled ? (icon as { d: string }).d : (icon as string)
  const fill = isFilled ? ((icon as { fill?: string }).fill ?? 'none') : 'none'
  const stroke = isFilled ? ((icon as { stroke?: string }).stroke ?? 'currentColor') : 'currentColor'
